...
describe("io/unicode.ts", function () {
    it("preserves empty and whitespace strings", function () {
        chai_1.expect(unicode_1.default.latin2utf8("")).to.equal("");
        chai_1.expect(unicode_1.default.latin2utf8("  \t ")).to.equal("  \t ");
        chai_1.expect(unicode_1.default.latin2utf8("  \r\n\t\r\n ")).to.equal("  \r\n\t\r\n ");
    });
    it("preserves ascii strings", function () {
        var ascii_chars = lodash_1.range(32, 127)
            .map(function (dec) { return String.fromCharCode(dec); })
            .join("\t");
        chai_1.expect(unicode_1.default.latin2utf8(ascii_chars)).to.equal(ascii_chars);
    });
    it("preserves native javascript code points", function () {
        var umlauts = "öäüÖÄÜß";
        chai_1.expect(unicode_1.default.latin2utf8(umlauts)).to.equal(umlauts);
    });
    it("converts pure utf-8", function () {
        chai_1.expect(unicode_1.default.latin2utf8("Ã¤Ã¶Ã¼ÃÃÃÃ")).to.equal("äöüÄÖÜß");
    });
    it("converts mixed utf-8", function () {
        chai_1.expect(unicode_1.default.latin2utf8("ÃtheriÃ¶l FuÃlÃ¤ufig BÃ¶ttcher")).to.equal("Ätheriöl Fußläufig Böttcher");
    });
        ...
        [0x10ffff, 0xffff, 0x7ff, 0x7f].forEach(function (point) {
            [1, 2, 3].forEach(function (divisor) {
                var ascii = code_to_utf8(point / divisor)
                    ...
                    .join("");
                var utf8 = String.fromCharCode(point / divisor);
                chai_1.expect(unicode_1.default.latin2utf8(ascii)).to.equal(utf8);
            });
        });
    });
});
